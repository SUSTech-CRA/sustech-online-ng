  ...
  <main class="bus-detail" :lang="busLanguage === 'zh' ? 'zh-CN' : 'en'">
    ...
    <template v-else-if="route">
      ...
      <section class="map-slot" :aria-label="label('map')">
        ...
        <BusMapV2 v-else :routes="[route]" :vehicles="vehicles" :stops="stops" :route-id="route.id" :language="busLanguage" />
      </section>
    </template>
  </main>
</template>
...
const labels = { zh: { route: '线路', home: '返回首页', saved: '已收藏', save: '收藏', noServiceTime: '未提供服务时间', timetable: '时刻表', direction: '方向', close: '收起', allSchedules: '查看全部时刻表', selectedStop: '当前站点', allArrivals: '查看全部到站', collapse: '收起', stops: '站点', vehicles: '车辆', map: '线路地图', normal: '正常运营', suspended: '暂停运营', updated: '更新于 {time}', planned: '计划时间', realTime: '实时预测', minutes: '{minutes} 分钟' }, en: { route: 'Route', home: 'Back to home', saved: 'Saved', save: 'Save', noServiceTime: 'Service time unavailable', timetable: 'Timetable', direction: 'Direction', close: 'Close', allSchedules: 'All schedules', selectedStop: 'Selected stop', allArrivals: 'All arrivals', collapse: 'Show less', stops: 'Stops', vehicles: 'vehicles', map: 'Route map', normal: 'In service', suspended: 'Suspended', updated: 'Updated {time}', planned: 'Scheduled time', realTime: 'Real-time prediction', minutes: '{minutes} min' } }
...
function arrivalText(item) { return arrivalSource(item) === 'real_time' || arrivalSource(item) === 'realtime' ? [label('minutes', { minutes: item.eta_minutes }), arrivalDistance(item)].filter(Boolean).join(' ') : arrivalSource(item) === 'planned' ? busText('planAt', { time: time(item.planned_arrival_at) }) : busText(unavailableReasonTextKey(item.unavailable_reason)) }
...
async function refresh() { const routeId = route.value?.id; refreshRemaining.value = 30; now.value = new Date(); if (!routeId) return; try { const [routeData, noticeData, vehicleData] = await Promise.all([publicApi(`/routes/${encodeURIComponent(routeId)}`), busApi.notices(), busApi.vehicles()]); route.value = routeData; notices.value = Array.isArray(noticeData) ? noticeData : []; vehicles.value = (Array.isArray(vehicleData) ? vehicleData : []).filter((vehicle) => vehicle.route_id === routeId) } catch { /* retain the last usable data */ } await loadArrivals() }
...
defineExpose({ load, refresh, selectDirection, selectStop })
</script>
...
.route-stops .vehicle-icon { z-index: auto; display: block; width: 1.25rem; height: 1.25rem; margin: 0; border: 2px solid var(--route-color); border-radius: 50%; background: var(--route-color) url('/bus.png') center / contain no-repeat; box-shadow: 0 0 0 1px var(--route-color); }
.route-stops .vehicle-icon.shuttle { background-image: url('/sev.png'); }
.head-actions { gap: .3rem; }
.plain-button, .selected-stop-title a { color: inherit; text-decoration: none; }
.selected-stop-title a:hover, .selected-stop-title a:focus-visible { color: var(--bus-v2-link); text-decoration: underline; }
.bus-detail { color: var(--bus-v2-text); }
.bus-detail .eyebrow, .bus-detail .muted, .bus-detail .arrival-list small, .bus-detail .notices summary time, .bus-detail .notices summary > span::after { color: var(--bus-v2-muted); }
.bus-detail .panel { border-color: var(--bus-v2-border); background: var(--bus-v2-bg); }
.bus-detail .plain-button, .bus-detail .direction-tabs button, .bus-detail .arrival-list li, .bus-detail details + details { border-color: var(--bus-v2-border); }
.bus-detail .direction-tabs .active { background: var(--bus-v2-link-soft); color: var(--bus-v2-link); }
</style>
