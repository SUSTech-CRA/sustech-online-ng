<template>
  <main class="bus-vehicles" :lang="language === 'zh' ? 'zh-CN' : 'en'">
    ...
    <template v-else>
      ...
      <BusMapV2 :routes="routes" :vehicles="vehicles" :language="language" />
...
const language = busLanguage; const routes = ref([]); const vehicles = ref([]); const allNotices = ref([]); const loading = ref(true); const error = ref('')
let refreshTimer
...
async function load() { loading.value = true; error.value = ''; try { const [routeData, vehicleData, noticeData] = await Promise.all([busApi.routes(), busApi.vehicles(), busApi.notices()]); routes.value = Array.isArray(routeData) ? routeData : []; vehicles.value = Array.isArray(vehicleData) ? vehicleData : []; allNotices.value = Array.isArray(noticeData) ? noticeData : [] } catch (reason) { error.value = reason.message || String(reason) } finally { loading.value = false } }
